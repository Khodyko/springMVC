...
public class TicketServiceImplTest extends TestCase {
    ...
    @Test
    public void testBookTicket() throws ServiceException {
        TicketEntity ticketEntity = new TicketEntity(12, 12, 12,
                                                    Ticket.Category.BAR, 12);
        UserEntity userEntity = new UserEntity(12, "Sergei", "sergei@mail.ru");
        EventEntity eventEntity = new EventEntity(12, "title12",
                                                    new Date(System.currentTimeMillis()));
        when(ticketDaoImpl.saveBookedTicket(12, 12, 12,
                                            Ticket.Category.BAR)).thenReturn(ticketEntity);
    ...
    @Test
    public void testGetBookedTicketsByEvent() throws DaoException, ServiceException {
        ...
        when(ticketDaoImpl.getBookedTicketsByEvent(eventIdSet, 100,
                                            0)).thenReturn(ticketList);
        assert (ticketServiceImpl.getBookedTickets(eventEntity, 100,
                                            0).get(0).equals(ticketEntity));
    }

    @Test
    public void testGetBookedTicketsByUser() throws DaoException, ServiceException {
        ...
        when(ticketDaoImpl.getBookedTicketsByUser(userIdSet, 100,
                                        0)).thenReturn(ticketList);
        assert (ticketServiceImpl.getBookedTickets(userEntity, 100,
                                        0).get(0).equals(ticketEntity));
}
